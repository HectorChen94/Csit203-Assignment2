...
void EventQueue::addE(event ev) {
    ...
    assert (actualE < expectedE);  // otherwise beyond range of events[]
...
int ServerPool::assign(float thisStart, float thisDuration) {
    ...
    assert(serversBusy < serverCount);  // should not get here unless there is an idle server
...
void ServerPool::release(int assignedServer) {
    ...
    assert(serversBusy > 0); // should not get here unless there is at least one server working
